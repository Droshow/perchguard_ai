...
# ─── EC2 Node Group Role (Phase 10c — isolated-agent node group) ─────────────
# Different trust principal from the Fargate execution role above: EC2 worker
# nodes assume this directly, not via the eks-fargate-pods service.

resource "aws_iam_role" "agent_node_group" {
...
resource "aws_iam_role_policy_attachment" "agent_node_worker_policy" {
  role       = aws_iam_role.agent_node_group.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonEKSWorkerNodePolicy"
}

resource "aws_iam_role_policy_attachment" "agent_node_cni_policy" {
  role       = aws_iam_role.agent_node_group.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonEKS_CNI_Policy"
}

resource "aws_iam_role_policy_attachment" "agent_node_ecr_readonly" {
  role       = aws_iam_role.agent_node_group.name

...
```bash
git clone https://github.com/perchguard/perchguard.git
cd perchguard
...
In observe mode the decision is `ALLOW`, but the audit log records what would have fired:
...
**Terminal dashboard** — polls the management API and renders a compact table:

```bash
PERCHGUARD_API_KEY=pgmk-...  # key from startup log
go run ./cmd --mode=watch --watch-key=$PERCHGUARD_API_KEY
```

**Budget meter** — per-session token / cost / call tracking:

```bash
go run ./cmd --mode=meter --watch-key=$PERCHGUARD_API_KEY
```

---

## 5. Copilot mode — govern your IDE agent

If you use GitHub Copilot, Claude Code, or another MCP-based IDE agent, start PerchGuard in copilot mode. It acts as an MCP proxy between your agent and its tool server, and opens a browser dashboard:

```bash
PERCHGUARD_MCP_UPSTREAM=http://your-mcp-server:3000 \
...
[perchguard] mcp proxy started (upstream=http://your-mcp-server:3000)
```

Open `http://localhost:8081` in a browser. The dashboard shows session budget (tokens used, estimated cost, call count) and the active policy set — refreshed every 3 seconds.

The copilot profile (`configs/copilot-profile.yaml`) is pre-tuned for local developer use: cost caps, secret redaction in tool outputs, prompt injection detection, and destructive command blocking. The semantic firewall and fleet analysis are disabled — the developer's presence replaces them.
...
Edit `configs/policies.yaml`, change the first line:

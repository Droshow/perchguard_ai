...
Four ways to wire an existing LLM agent to PerchGuard, in order of integration depth.
...
## Option 3 — Copilot Mode (IDE agents)

For local IDE agents (GitHub Copilot, Claude Code, Cursor) that speak MCP over stdio, use copilot mode. PerchGuard wraps the MCP subprocess and adds governance with no agent changes.

```bash
PERCHGUARD_MCP_UPSTREAM=http://your-mcp-server:3000 \
  ./perchguard --mode=copilot
```

Copilot mode:
- Loads `configs/copilot-profile.yaml` automatically (cost caps, secret redaction, destructive command blocking)
- Starts a browser dashboard at `http://localhost:8081` showing session budget and active policies
- Runs as an MCP stdio proxy — configure your IDE to use the PerchGuard binary as the MCP command

To point Claude Code at PerchGuard, in `.vscode/mcp.json`:

```json
{
  "mcpServers": {
    "perchguard": {
      "command": "/path/to/perchguard",
      "args": ["--mode=copilot"],
      "env": {
        "PERCHGUARD_MCP_UPSTREAM": "http://localhost:3000"
      }
    }
  }
}
...
The `--mode=wrap` flag auto-discovers `.vscode/mcp.json`, patches the upstream URLs, and restores the file on exit — zero manual configuration.

---

## Option 4 — Registered Agent (Declared Intent)
...
| Existing HTTP MCP agent, minimal changes | MCP proxy |
| IDE agent (Copilot, Claude Code, Cursor) | Copilot mode |

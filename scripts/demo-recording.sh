...
#   bash scripts/demo-recording.sh              # Run demo (services stay up)
#   bash scripts/demo-recording.sh --clean      # Clean up services after recording
#   bash scripts/demo-recording.sh --help       # Show help
#
# Prerequisites:
...
# Handle --help flag
if [[ "${1:-}" == "--help" ]]; then
  printf "Usage: bash scripts/demo-recording.sh [OPTIONS]\n\n"
  printf "Options:\n"
...
cleanup() {
  ...
  printf "  bash scripts/demo-recording.sh --clean\n\n"
...
printf "  ${YELLOW}bash scripts/demo-recording.sh --clean${NC}\n"
